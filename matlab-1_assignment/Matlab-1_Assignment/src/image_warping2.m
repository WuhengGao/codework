...
subplot(1,3,2); 
himg = surface(repmat(j,h,1),repmat(i,1,w),ones(h,w),reshape(1:h*w,h,w),'FaceColor','texturemap','EdgeColor','none','CDataMapping','direct');
colormap(map);
axis square;
title({'Warpped Image', 'RBF'});
subplot(1,3,3); 
himg2 = surface(repmat(j,h,1),repmat(i,1,w),ones(h,w),reshape(1:h*w,h,w),'FaceColor','texturemap','EdgeColor','none','CDataMapping','direct');
colormap(map);
axis square;
